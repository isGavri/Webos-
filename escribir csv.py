if __name__== "__main__":
    ...
    with open('Egg Grade Dataset.csv',newline='',encoding="utf-8-sig") as csvLectura:
        ...
        for row in lectura:
            row["Stain_Area"] = round(random.uniform(0.0,9.3),2)
            ...
            row["Specific_gravity"]=round(random.uniform(1.050,1.090),3)
            ...
            row["Average_rugosity"]=round(random.uniform(0.1,10.5),1) # micrómetros
            row["Eggshell_thickness"]=round(random.uniform(0.30, 0.45),3)

            filas.append(row)
        columnas = ["Height","Diameter","Weight","Stain_Area","Eggshell_fissure","Leaks_presence","Internal_immaculacy","Specific_gravity","Motted_area","Internal_chamber_distance","Color_uniformity","Average_rugosity","Eggshell_thickness"]
